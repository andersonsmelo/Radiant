...
| Play: verificação de desenvolvedor | Brasil é um dos quatro primeiros países (com Indonésia, Singapura e Tailândia): a partir de **30/09/2026**, app instalado em aparelho Android certificado precisa estar registrado por desenvolvedor verificado, e o registro é do par **nome do pacote + fingerprint SHA-256 da chave de assinatura** | `com.ascendcreative.radiant` provavelmente entrou no registro automático do Play (>99% dos apps com Play App Signing), mas **isso precisa ser conferido na home do Play Console** — a consequência declarada de não registrar é remoção global, não atrito. Risco separado e não medido: os perfis de *internal distribution* do EAS instalam fora do Play assinados por outra keystore, ou seja, outro par pacote+fingerprint — se ele não for registrado, o que quebra é a distribuição de beta interna, não a publicação. Ver item 8 da [fila](../FILA.md) |
...
- **K1 [CONCLUÍDA — LOCAL, 2026-09-14]** `NextNodeResolver` e integração ao
  snapshot/contrato de rota entregues com 9/9 casos do motor e 25/25 casos de
