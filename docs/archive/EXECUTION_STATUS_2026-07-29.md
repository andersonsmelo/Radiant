...
apagados. **A Task 4, grafo das 30 competências, foi concluída em 2026-08-01** —
30 competências em 6 unidades, `cycleCount: 0`, `orphanCount: 0`, 10 marcadas como
críticas de segurança, com validador próprio ligado ao `validate-foundation`. Ela
não dependia do lote de mídia: o gate da Fase 0 lista as duas coisas como
...
### Verificações de 2026-08-01 — currículo, entitlement e reconciliação do repositório
...
| Task 9 — registro de renderizadores e player desacoplado | **CONCLUÍDA (2026-08-02)** | O player deixou de saber quais tipos de atividade existem: pede o renderizador ao registro e passa um contrato único `interaction/value/onChange`. Estado do percurso saiu da tela para `useLearningActivity`, testável sem montar UI. **O `LessonFlowScreen.flow.test.tsx` passou sem uma linha alterada** — é a evidência de que o comportamento do player não mudou. Tipo sem renderizador devolve aviso em vez de derrubar a lição, porque conteúdo é versionado separadamente do app. 16 testes novos; `npm run quality` em **42 suítes / 211 testes**; paridade de fluxos 6/15 |
| Task 8 — domínio por competência | **CONCLUÍDA (2026-08-02)** | `calculateCompetencyMastery` puro e determinístico — sem relógio, sem storage, limiares e datas por parâmetro. Cinco estados, pesos por tipo de evidência, degradação por erro recente, `mastered` bloqueado sem retenção e **gate crítico de segurança assimétrico**: erro recente rebaixa um nível numa competência comum e trava em `practicing` numa competência crítica. **Evidência legada é ignorada por padrão** — não sabe qual competência mede, e somá-la daria número plausível e errado. Migração vazia: ausência é `not-started`, e o repositório lê só a própria chave; nada infere domínio de XP ou nó concluído. 28 testes novos; `npm run quality` em **40 suítes / 195 testes** |
| Task 7 — evidência estruturada por interação | **CONCLUÍDA (2026-08-02)** | `LearningEvidenceRepository` grava uma evidência por interação em chave versionada, com teto de 1.000 e remoção dos mais antigos. **Fronteira de privacidade estrutural:** os nove campos da allowlist são ids de forma fixa, enums e timestamp ISO — não existe campo para prosa, então resposta livre não tem onde caber; varredura de URI e marcador clínico é a segunda camada. Escrita **recusa** registro inválido sem gravar; falha de storage vira log e não derruba a conclusão. 20 testes novos; `npm run quality` em **38 suítes / 167 testes** |
| Task 6 — adaptador do catálogo legado | **CONCLUÍDA (2026-08-02)** | `LegacyLessonAdapter` puro converte bloco legado em atividade v2 preservando número e ordem das telas, ids, resposta correta, explicação e `lessonId`. Os quatro blocos reais adaptam sem violação. 12 testes novos; `lesson-flow` em **50 testes / 4 suítes**; paridade `npm run test:flows` em **6 suítes / 15 testes sem atualizar snapshot**. Evidência legada marcada como `legacy-lesson-recall` e competência sintética com prefixo `competency:legacy:` — conteúdo legado nunca foi escrito contra o currículo, e atribuir competência real contaminaria o cálculo de domínio de forma invisível |
...
| arquivo novo invisível ao git na árvore de conteúdo | **DIAGNOSTICADA** | `.git/info/exclude` contém `Conte*do/`. Arquivo novo naquela árvore não aparece nem como não rastreado, enquanto os irmãos seguem visíveis porque ignore não afeta arquivo já rastreado. Os dois JSON do currículo exigiram `git add -f`. Vale para todo arquivo de conteúdo novo, de qualquer sessão |
| `scripts/content/foundation-structure.test.mjs` | **FALHA — e nenhum gate o executa** | 1 de 2 testes falha: espera `conteúdo/` minúsculo no `README.md`, que escreve `Conteúdo/`. Falha também em `HEAD`, portanto é anterior a esta sessão. Não está nos validadores do `.loop/project.yaml` nem em script algum de `package.json` — *um teste fora de todo gate não é rede de proteção, é dívida silenciosa* |
